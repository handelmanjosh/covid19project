...
def testCovidContact(delayTime,period,effectivity,traceTime, positivityRate, testAmount): #end of day testing with contact tracing
...
def runTest(MAX, DAYS, startInfect, cohortNum, maskEffectivity, delayTime, gymTest, positivityRate, testAmount, vaxAmount, testNum):
  filenameTemp = "days-"+ str(DAYS) + " starting infected-" +str(startInfect) + " cohorts-"  + str(cohortNum) +  " mask-"+ str(maskEffectivity) + " delay time-"  + str(delayTime)+ " gymTest-" + str(gymTest) + " positivity rate-" + str(positivityRate) + " test amount-" + str(testAmount) + " vax amount-" + str(vaxAmount)
  ...
  for i3 in range(MAX): #number of independent runs
    ...
    for i43 in range(DAYS): #number of days. Changeable
      for i6 in range(27): #number of periods in a day, do not change (9 periods, broken into 3 15 min blocks)
        ...
        indivLockdown()
        infectorGrower()
      checkLockdown(delayTime, positivityRate, testAmount)
      billGates(delayTime, vaxAmount)
...
if __name__ == "__main__":
    ...
    for i in range(outsideIterations):
      ...
      for i2 in range(insideIterations):
        p = multiprocessing.Process(target = runTest, args=(MAX1, DAYS1, startInfect, cohortNum, maskEffectivity, delayTime, gymTest, positivityRate, testAmount, vaxAmount, counter2))
        p.start()
        processList.append(p)
        labels.append("M:"+ str(round(maskEffectivity, 2)) + "\nVX:" + str(round(vaxAmount,2)))
        vaxAmount = vaxAmount + 20
        counter2 += 1
        controlList.append(counter2) 
      maskEffectivity = maskEffectivity + .1
